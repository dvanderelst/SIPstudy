...
def regression(data, column, alpha):
    ...
    # Build a DataFrame with named columns for predictors
    X = pd.DataFrame({
        "const": 1,  # intercept
        "days": x
    })
    ...
    # Now params, pvalues, bse all have index ["const", "days"]
    slope = results.params["days"]
    ...
    return {
        ...
        "p_value": results.pvalues["days"],
        "standard_error": results.bse["days"],
        "residuals": residuals,
        "model": model,
        "results": results,
